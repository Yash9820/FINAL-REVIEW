...
application.config["SQLALCHEMY_DATABASE_URI"] = 'mysql+mysqlconnector://admin:admin123@database-1.csz9pntrz3tv.ap-south-1.rds.amazonaws.com:3306/student'.format(user='admin', password='admin123', server='database-1.csz9pntrz3tv.ap-south-1.rds.amazonaws.com', database='student')
application.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = True
application.config['MYSQL_HOST'] = 'database-1.csz9pntrz3tv.ap-south-1.rds.amazonaws.com'
...
def viewassignmentsb(id):
	if g.email:
		...
		data = cur.fetchall()
		cur.close()
		return render_template('/StudentLogin/viewfullassign.html',showstsb = data, showans=result	)
	return redirect(url_for('login'))
		...
		if request.method=='POST':
			...
			if file and allowed_file(file.filename):
				...
				if result>0:
					...
					if assid and enroll :
						cur.execute("""UPDATE uploadassign SET file=%s,uploaddate=%s WHERE Enrollment=%s AND asid=%s""", (file.filename, formatted_date, session['Enrollment'],id))
...
def too_large(e):
	...
	return render_template('/TeacherAdmin/CreateAssignment.html')
